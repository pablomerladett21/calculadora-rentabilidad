...
const MOVEMENT_LABELS: Record<string, { label: string; icon: React.ReactNode; color: string }> = {
  ...
  adjustment: { label: 'Corrección', icon: <SlidersHorizontal size={14} />, color: 'text-amber-500' },
}
...
export default function StockPage() {
  ...
  const fetchData = async () => {
    ...
    const [productsRes, movementsRes] = await Promise.all([
      supabase.from('products_roi').select('id, product_name, stock_quantity, stock_alert_threshold').eq('user_id', user.id).order('product_name'),
      supabase.from('stock_movements').select('*, products_roi(product_name)').eq('user_id', user.id).order('created_at', { ascending: false }).limit(100),
    ])
  ...
  const filteredMovements = selectedProductFilter 
    ? movements.filter(m => m.product_id === selectedProductFilter)
    : movements

  const noStockCount = products.filter(p => (p.stock_quantity ?? 0) === 0).length
  const lowStockCount = products.filter(p => {
    const s = p.stock_quantity ?? 0
    const t = p.stock_alert_threshold ?? 5
    return s > 0 && s <= t
  }).length
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg">Monitoreá el inventario y el historial de movimientos.</p>
      </div>
            ...
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <th className="px-6 py-4 font-black text-[10px] text-slate-400 uppercase tracking-widest text-center">Acción</th>
                </tr>
              </thead>
          ...
          <table className="w-full text-left text-sm border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredMovements.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-16 text-center text-slate-400 font-medium">Sin movimientos registrados aún.</td>
                </tr>
              ) : filteredMovements.map(mov => {
                const meta = MOVEMENT_LABELS[mov.movement_type]
                return (
                  <tr key={mov.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-6 py-3 text-sm text-slate-400 font-medium">{mov.reason || '—'}</td>
                  </tr>
                )
              })}
            </tbody>
